...
class SuccessCriterion:
    ...
    def post_process_test_result(self, test_result):
        ...
        # test_result[SAMPLE_SIZE_SUFFICIENT_STR] will always be True for Bayesian algorithms
        # This is because the minimum sample size requirement for bayesian algorithms is 0 (check experiment) for these algorithms
        # So the string "Insufficient sample size. " is never appended to the conclusion string
        conclusion_str = ["Insufficient sample size. " if not test_result[SAMPLE_SIZE_SUFFICIENT_STR] else result_str]
        # counter is exceeded if:
        # 1. Metric is a counter metric, 2. Criterion is of type threshold, and 3. The Success Criterion test has failed
        # 4. We also make sure that success criteria did not fail because of insufficient sample size (this is always True for Bayesian Algorithms)
        ...
        return {
            ...
            # The following key value pair will not be used for Bayesian Algorithms
            # It has been left as is for code reusability
